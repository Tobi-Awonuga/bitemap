...
export default function AdminUsersPage() {
  ...
  const loadUsers = (nextOffset = 0, nextSearch = search, nextRole = roleFilter) => {
    setLoading(true)
    ...
    api
      .get<AdminUsersResponse>(`/api/admin/users?${params.toString()}`)
      .then((res) => {
        setUsers(res.data)
  ...
  useEffect(() => {
    const timer = setTimeout(() => loadUsers(0, search, roleFilter), 250)
  ...
  const toggleRole = async (user: AdminUser) => {
    ...
    try {
      const updated = await api.patch<Pick<AdminUser, 'id' | 'role'>>(`/api/admin/users/${user.id}`, { role: newRole })
      setUsers((prev) => prev.map((current) => (current.id === user.id ? { ...current, ...updated } : current)))
      setError(null)
    } catch (err) {
  ...
  const deleteUser = async (user: AdminUser) => {
    ...
    try {
      ...
      loadUsers(Math.max(0, Math.min(offset, Math.max(total - 1, 0))), search, roleFilter)
      setError(null)
    } catch (err) {
    ...
    <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isCurrentUser && <span className="text-xs text-slate-500 shrink-0">(you)</span>}
                        </div>
      ...
      {!loading && total > 0 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => loadUsers(Math.max(0, offset - limit), search, roleFilter)}
              disabled={offset === 0}
            ...
            <button
              onClick={() => loadUsers(offset + limit, search, roleFilter)}
